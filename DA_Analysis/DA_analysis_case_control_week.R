...
maxL2FC <- 4
myLtsr <- as.data.frame(ltsr[,14:18])
myPostmean <- as.data.frame(postmean[,14:18])
...
write.table(longPost,"Rik_model_results.tsv",col.names=TRUE,row.names=FALSE,quote=FALSE,sep="\t")
...
ggplot(df[df$sampleInfo!="3",],aes(x=sampleInfo,y=label,col=post,size=-log10(fdr))) + 
    ...
    theme_classic()+
    #theme_rik(base_family = "Helvetica") + 
...
ggsave("Riks_code_no_IFN_adjust_size.pdf",height=8,width=6)
