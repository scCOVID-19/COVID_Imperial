...
y$samples$case_control <- ordered(y$samples$case_control)
# Estimate Dispersion
...
results1 <- glmmSeq(~ case_control + sex + ethnicity + calc_age + (1|individual_id),
                  ...
                  designMatrix = model.matrix(~ case_control + sex + ethnicity + calc_age, data=y$samples),
                  removeSingles=FALSE,
                  progress=TRUE,
                  cores = opt$NCPU)
results2 <- glmmSeq(~ case_control + sex + ethnicity + calc_age + days_to_admission + (1|individual_id),
                  ...
                  dispersion = disp,
                  sizeFactors = sizeFactors,
                  designMatrix = model.matrix(~ case_control + sex + ethnicity + calc_age + days_to_admission, data=y$samples),
                  removeDuplicatedMeasures = FALSE,
                  removeSingles=FALSE,
                  progress=TRUE,
                  cores = opt$NCPU)
save(results1, results2, file = opt$OUT)