...
testDGElist2 <- function(dgelist, formula, individual_id, modified = FALSE, optimizer = "bobyqa",
    designMatrix = NULL, ncores = NULL, BPPARAM = SerialParam(progress = TRUE), tpm = FALSE,
    ...) {
    if (is.null(ncores)) {
        NCORES <- parallel::detectCores() - 1
    ...
    if (modified) {
        results <- suppressMessages(glmm_modified2(dgelist, modelFormula = formula,
            dispersion = disp, sizeFactors = sizeFactors, id = individual_id, control = glmerControl(optimizer = optimizer,
                optCtrl = list(maxfun = 2e+05)),  ...)) #BPPARAM = BPPARAM,
        results <- glmm_qval(results, pi0 = 1)
    } else {
...
testDGElist3 <- function(dgelist, formula, individual_id, modified = FALSE, optimizer = "bobyqa",
    designMatrix = NULL, ncores = NULL, BPPARAM = SerialParam(progress = TRUE), tpm = FALSE,
    ...) {
    if (is.null(ncores)) {
        NCORES <- parallel::detectCores() - 1
    ...
    if (modified) {
        results <- suppressMessages(glmm_modified3(dgelist, modelFormula = formula,
            dispersion = disp, sizeFactors = sizeFactors, id = individual_id, control = glmerControl(optimizer = optimizer,
                optCtrl = list(maxfun = 2e+05)),  ...)) #BPPARAM = BPPARAM,
        results <- glmm_qval(results, pi0 = 1)
    } else {
...
glmm_modified2 <- function(dgeList, modelFormula, id, dispersion, sizeFactors = NULL,
    control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e+05)),
     verbose = TRUE, cores = 1, ...) { #BPPARAM = SerialParam(progress = TRUE),
    metadata <- dgeList$samples
    countdata <- dgeList$counts
    ...
    resultList <- pbmclapply(fullList, function(geneList) {
        try(run_glmm2(data = metadata, geneList = geneList, fullFormula = fullFormula,
            control = control, offset = offset, ...), silent = TRUE)
    }, mc.cores = cores) #BPPARAM = BPPARAM
    # Print timing if verbose
    end <- Sys.time()
...
glmm_modified3 <- function(dgeList, modelFormula, id, dispersion, sizeFactors = NULL,
    control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e+05)),
     verbose = TRUE, cores = 1, ...) { #BPPARAM = SerialParam(progress = TRUE),
    metadata <- dgeList$samples
    countdata <- dgeList$counts
    ...
    resultList <- pbmclapply(fullList, function(geneList) {
        try(run_glmm3(data = metadata, geneList = geneList, fullFormula = fullFormula,
            control = control, offset = offset, ...), silent = TRUE)
    }, mc.cores = cores) #BPPARAM = BPPARAM
    # Print timing if verbose
    end <- Sys.time()
...
run_glmm2 <- function(data, geneList, fullFormula, control, offset, ...) {
    data[, "count"] <- as.numeric(geneList$y)
    fit <- lme4::glmer(fullFormula, data = data, control = control, offset = offset,
        family = MASS::negative.binomial(theta = 1/geneList$dispersion), ...)
     if (class(fit) != "try-error") {
        ...
        if (class(wald) != "try-error") {
            ...
            singular <- as.numeric(isSingular(fit))
            conv <- length(slot(fit, "optinfo")$conv$lme4$messages)
            return(list(stats = c(stats, fixedEffects,se, waldtest), fit = fit, optinfo = c(singular,
                conv), tryErrors = ""))
        } else {
            return(list(stats = NA, fit = NA, optinfo = NA, tryErrors = wald[1]))
        }
...
run_glmm3 <- function(data, geneList, fullFormula, control, offset, ...) {
    data[, "count"] <- as.numeric(geneList$y)
    fit <- lme4::glmer(fullFormula, data = data, control = control, offset = offset,
        family = MASS::negative.binomial(theta = 1/geneList$dispersion), ...)
     if (class(fit) != "try-error") {
        stats <- setNames(c(geneList$dispersion, AIC(fit), as.numeric(logLik(fit))),
            c("Dispersion", "AIC", "logLik"))
        fixedEffects <- lme4::fixef(fit)
        se<-summary(fit)$coefficients[4,2]
        se<-setNames(se,"SE")
        wald <- try(car::Anova(fit), silent = TRUE)
        if (class(wald) != "try-error") {
            waldtest <- setNames(c(wald[, "Chisq"], wald[, "Pr(>Chisq)"]), c(paste0("Chisq_",
